# LNG市场分析助手 Skill v4.9.2 - xcrawl爬虫优化版

**版本**: v4.9.2 (xcrawl爬虫优化版)  
...
- ✅ **10位助理个性化工具策略** - 含预计耗时和优化建议
- ✅ **整体采集时间优化** - 预计5-6分钟完成全部采集
...
### 报告文件规范
每日报告必须输出以下三种格式到 `reports/lng/daily/YYYY-MM-DD/` 文件夹：

| 文件名 | 格式 | 用途 |
|--------|------|------|
| `report.md` | Markdown | 原始数据+分析，便于版本控制 |
| `report.html` | HTML | 可视化展示，网页浏览 |
| `data.json` | JSON | 结构化数据，程序化处理 |
...
2. **HTML网页版**: 可视化展示
3. **JSON数据版**: 结构化数据，便于汇总分析
4. **国内价格板块**: 细化展示液厂+接收站+变动
5. **浙江专区板块**: 细化展示4接收站（含浙能温州）+周边+动态+分析
6. **来源标注**: 每个数据必须标注来源和置信度
7. **中孚审核**: 审核报告必须附在最终报告后
8. **【v4.9.1新增】Token日志**: 记录markdown-fetch节省的Token
...
### 助理快速参考
每位助理采集前，先查阅 `TOOLS_KIT.md` 中的对应配置：
- 原油助理 → 查看"原油助理"配置
- 润仓助理 → 查看"润仓助理（★必采项★）"配置
- 以此类推
